...
const ManageUserAccounts = ({backendURL, CancelIcon, EditIcon, generateRandomPassword, SaveIcon, setIsAdding, setIsEditing, validatePassword}
  :
  {backendURL: string, CancelIcon: typeof MuiIcon, EditIcon: typeof MuiIcon, generateRandomPassword: () => string, SaveIcon: typeof MuiIcon, setIsAdding: (arg0: boolean) => void, setIsEditing: (arg0: boolean) => void, validatePassword: (arg0: string) => boolean}
  ) => {
  ...
  const saveNewPassword = () => {
    ...
    if (!dialogVisibleParamIsNew) {
      axios
        .put(`${backendURL}/UpdateUser?wl_userid=${dialogVisibleParamID}&wl_password=${encodeURIComponent(newPassword)}`, { withCredentials: true })
  ...
  useEffect(() => {
    if (!usersLoadingStarted && !usersLoadingComplete) {
      ...
      axios
        .get(`${backendURL}/GetUsers`, { withCredentials: true })
        .then((res: typeof IUser) => {
