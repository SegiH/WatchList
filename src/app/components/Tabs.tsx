                    ...
                    <div className={`tabBar ${!darkMode ? "lightMode" : "darkMode"}`}>
                         {Object.keys(routeList)
                              .filter((routeName) => {
                                   ...
                                   && (routeName !== "BugLogs" || (routeName ==="BugLogs" && !demoMode && isAdmin() === true && String(process.env.NEXT_PUBLIC_ENABLE_BUGLOGS) === "true" && visibleSections.filter((section) => { return section.name === "BugLogs"}).length > 0))  // You cannot dynamically set Enabled on this route so don't call isEnabled()
